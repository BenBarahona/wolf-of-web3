...
/// @title Wolf Base Strategy Vault
/// @notice Generic ERC4626 strategy vault over USDC with cooldown, TVL cap and manual yield injection.
/// @dev Used as the core for Low / Medium / High risk strategies on different chains.
contract WolfBaseIndexVault is ERC4626, Ownable {
    ...
    /// @param usdc Address of the USDC token on the target chain.
    ...
    function accrueYield(uint256 amount) external onlyOwner {
        ...
        IERC20 usdc = IERC20(asset()); // cast address(asset()) -> IERC20
...
/*//////////////////////////////////////////////////////////////
                    HIGH-RISK MEME INDEX STRATEGY (WORLD)
//////////////////////////////////////////////////////////////*/

/// @title Wolf High-Risk Meme Index Vault (World)
/// @notice High-risk strategy vault on World Chain where users deposit USDC
///         and get exposure to a simulated "meme / degen index" managed by the AI agent.
/// @dev The actual index composition + trading is handled off-chain/by the agent.
///      On-chain, yield is modeled via `accrueYield`, and a longer cooldown enforces stickier capital.
contract HighRiskMemeIndexVault is WolfBaseIndexVault {
    constructor(IERC20 usdc, address owner_)
        WolfBaseIndexVault(
            ...
            "wMEME-WORLD",                    // symbol_
            owner_,                           // owner_
            14 days                           // cooldown_ (tune this as you like)
        )
    {}
}